...
const WALLET_ADDR_CACHE_MAX = 500; // hard cap for walletId Ú∆“ address lookups
...
async function updateLeaderboard() {
  try {
    ...
    // Does NOT accumulate all trades in a single array Ú¿‘ each page is
    ...
    // Aggregate stats per user as we stream Ú¿‘ no trade array accumulation
    const userStats = new Map(); // userId Ú∆“ {volume, trades, pnl, markets: Set, resolvedWins, resolvedLosses, profitableMarkets: Set}
    const PAGE = 1000;
    for (let from = 0; ; from += PAGE) {
      ...
      // Process this page immediately Ú¿‘ aggregate into userStats, then let GC reclaim
      for (const t of (data || [])) {
      ...
      try {
        ...
        for (const [marketAddress, mp] of s.marketPnL.entries()) {
          ...
          try {
            ...
            // On-chain read failed Ú¿‘ can't compute position shares precisely.
        ...
        // Free marketPnL Ú¿‘ no longer needed
        s.marketPnL.clear();
        s.marketPnL = null;
