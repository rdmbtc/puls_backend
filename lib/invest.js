    ...
    try {
      ...
      const cards = await Promise.all(
        AGENTS.map(async (a) => {
            ...
            const pool = roundUsdc(invs.reduce((s, i) => s + Number(i.amount_usdc || 0), 0));
  ...
  app.post('/api/invest/withdraw', auth || ((_req, _res, next) => next()), async (req, res) => {
    try {
      ...
      try {
        ...
        if (balance < claimableMicro) {
          return res.status(400).json({ error: 'Treasury balance too low for payout', balanceUsdc: Number(balance) / 1e6 });
        }
