...
export function registerInvest(app, deps) {
  ...
  const creditInvestment = async ({ agentId, investor, amountUsdc, paymentId }) => {
    ...
    if (funded) {
      await supabase.from('investments').update({ funded: true }).eq('id', id).catch((e) =>
      ...
      res.json({
        ...
        agents: cards.map(({ agent, invs, invested }) => {
          ...
          return {
            ...
            // Signed — negative means the agent is currently losing money.
            apyEstimatePct: pool > 0 ? roundUsdc((net / pool) * 12 * 100) : 0,
          };
        }),
