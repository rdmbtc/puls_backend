...
// ── RAIL CONTRACT ─────────────────────────────────────────────────────────────
// Vault DEPOSITS may originate from EITHER rail, decided by chooseRail():
//   'sca'          → dev-controlled wallet (gasless Circle SCA transfer)
//   'agent-wallet' → Circle Agent Wallet (plain USDC transfer via Circle CLI)
// Withdrawals ALWAYS execute treasury-side (admin EOA pays the investor) —
// the investor never signs a payout.
...
export function registerInvest(app, deps) {
  ...
  app.post('/api/invest/:agentId', authenticateUser, requireVerifiedUser, strictLimiter, async (req, res) => {
    try {
      ...
      const rail = chooseRail(walletId, circleAgent.keyFromUser(userId));
      if (rail === 'none') return res.status(400).json({ error: 'No wallet for user' });
      ...
      if (rail === 'agent-wallet') {
        const awKey = circleAgent.keyFromUser(userId);
      ...
      // ── SCA deposit (authed) — backend pays from the user's gasless wallet ──
