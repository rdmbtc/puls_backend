...
export function registerInvest(app, deps) {
  ...
  app.post('/api/invest/:agentId', authenticateUser, requireVerifiedUser, strictLimiter, async (req, res) => {
    try {
      ...
      const id = `inv_${String(txId).replace(/[^a-zA-Z0-9]/g, '').slice(0, 40)}`;
      const { error } = await supabase
        .from('investments')
        .upsert({
          id,
          ...
          agent_id: agentId,
          amount_usdc: roundUsdc(amount),
          status: 'active',
        }, { onConflict: 'payment_id' });
      if (error) {
        console.error(`[invest] sca credit failed for ${agentId}:`, error.message);
      ...
      const pool = roundUsdc(agentBalance + invs.reduce((s, i) => s + Number(i.amount_usdc || 0), 0));
