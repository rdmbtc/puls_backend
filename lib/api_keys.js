...
export function registerApiKeys(app, deps) {
  const { supabase, authenticateUser, requireVerifiedUser, strictLimiter } = deps;
  const limiter = strictLimiter || ((_req, _res, next) => next());

  // POST /api/keys/generate — mint a new key. The raw key is returned ONCE.
  app.post('/api/keys/generate', authenticateUser, requireVerifiedUser, limiter, async (req, res) => {
    try {
  ...
  // GET /api/keys - list my keys (never the raw key or its hash).
  app.get('/api/keys', async (req, res) => {
    try {
      if (!req.headers.authorization) return res.json({ ok: true, keys: [] });
      let uid = null;
      try {
        await new Promise((resolve, reject) => {
          authenticateUser(req, res, (err) => (err ? reject(err) : resolve()));
        });
        uid = req.user ? `supabase_${req.user.id}` : null;
      } catch (_) {}

      if (!uid) return res.json({ ok: true, keys: [] });
  ...
  app.post('/api/keys/revoke', authenticateUser, requireVerifiedUser, limiter, async (req, res) => {
    try {
