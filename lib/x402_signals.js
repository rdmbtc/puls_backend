...
export function registerX402Signals(app, deps) {
  ...
  async function findPaymentTx(payer, creator, priceUsdc, excludeTxIds) {
    ...
    while (cursor >= fromBlock) {
      ...
      try {
        ...
        for (const log of logs) {
          ...
          if (value >= micro) return { txHash: txId, amountUsdc: Number(value) / 1_000_000 };
        }
      } catch (e) {
  ...
  app.get('/api/x402/signals/:id/claim', async (req, res) => {
    try {
      ...
      const hit = await findPaymentTx(payer, payTo, Number(r.price_usdc), (used || []).map((u) => u.tx_id));
