...
export function registerX402Signals(app, deps) {
  ...
  async function findPaymentTx(payer, creator, priceUsdc, excludeTxIds, minTimeMs = 0) {
    ...
    while (cursor >= fromBlock) {
      ...
      try {
        // Skip a chunk only when even its NEWEST block predates the claim —
        // then every older chunk does too.
        const chunkNewestTs = await blockTs(cursor).catch(() => 0);
        if (minTimeMs > 0 && chunkNewestTs && chunkNewestTs < minTimeMs) break;
