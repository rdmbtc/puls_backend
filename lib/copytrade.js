  ...
  async function mirrorBuyToFollowers(leaderUserId, trade) {
    try {
      ...
      for (const f of followers) {
        ...
        try {
          // EXPLICIT SKIP — Circle Agent Wallet followers: the copy mirror
          // (approve polling via circle.getTransaction + INITIATED-state
          // saveTrade bookkeeping) is dev-custody-coupled and not wired to the
          // Circle CLI rail yet. Say so instead of silently dropping them.
          const followerAwKey = circleAgent.isEnabledForUser(followerUserId);
          if (followerAwKey) {
            console.warn(`[agent-wallet:${followerAwKey}] copy-trade skipped — mirror execution requires a dev-controlled wallet (not wired to Agent Wallets yet)`);
            continue;
          }
