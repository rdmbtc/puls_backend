/**
 * Puls Streams — autonomous agent layer (RFB 4 + the 30% "agentic sophistication").
 *
...
const RISK_FRAC = 0.06;        // commit at most ~6% of bankroll over the horizon
const HORIZON_SEC = 60;
const MIN_RATE = 0.0005;
const MAX_RATE = 0.02;
...
export function registerStreamingAgent(deps) {
  ...
  if (ENABLED) {
    ...
    setInterval(runSession, INTERVAL_MIN * 60_000).unref?.();
  }
  console.log(`[stream-agent] autonomous streaming agent ${ENABLED ? 'ON' : 'OFF'} (every ${INTERVAL_MIN}m, ${TICK_SEC}s ticks)`);
