...
export function registerStreamingAgent(deps) {
  ...
  async function runSession() {
    ...
    try {
      ...
      // EXPLICIT SKIP — pay-per-second streams settle from the PAYER's
      // dev-controlled wallet (lib/streaming.js requires a Circle SCA
      // walletId for the batched on-chain settlement), which Agent-Stack
      // agents don't have. Route them out before any decision is made.
      const payerAwKey = circleAgent.isEnabledForUser(payer.user);
      if (payerAwKey) {
        console.warn(`[agent-wallet:${payerAwKey}] streaming skipped — per-second stream settlement requires a dev-controlled payer wallet (not wired to Agent Wallets yet)`);
        return;
      }
