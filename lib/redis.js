...
if (REDIS_URL) {
  try {
    ...
    // the TLS handshake validates the server identity; without it we must skip
    // verification because the Aiven cert is self-signed.
    ...
    redisClient.on('error', (err) => console.warn('[valkey] connection error:', err.message));
    redisClient.on('close', () => console.warn('[valkey] connection closed'));
...
export function cacheMiddleware(ttlSeconds = 60, keyPrefix = 'route') {
  return async (req, res, next) => {
    ...
    try {
      ...
      if (cachedData) {
        memCache.set(cacheKey, { at: now, data: cachedData });
      ...
      res.json = (body) => {
        if (res.statusCode >= 200 && res.statusCode < 300 && body) {
          memCache.set(cacheKey, { at: now, data: body });
...
// L1 in-memory TTL cache. Bounded: entries are small (a few KB — markets is
// the largest at ~76KB) and only exist while the route stays hot.
...
export function createValkeyRateLimitStore(windowMs, tag = '') {
  ...
  // Unique prefix per limiter: express-rate-limit forbids sharing a single
  // Store instance across multiple limiters (ERR_ERL_STORE_REUSE).
  ...
  return {
    ...
    async get(key) {
      const raw = await redisClient.get(bucketKey(key));
      if (raw == null) return undefined;
      return { totalHits: parseInt(raw, 10) || 0, resetTime: resetTime() };
    },
    async decrement(key) {
      const k = bucketKey(key);
      const v = await redisClient.decr(k);
      if (v < 0) {
        // Never go below zero — drop the key and let it restart fresh.
        await redisClient.del(k);
      }
    },
    async resetKey(key) {
      await redisClient.del(bucketKey(key));
    },
    async resetAll() {
