...
const SCAN_CHUNKS_MAX = parseInt(process.env.AGENT_REPUTATION_SCAN_CHUNKS || '40', 10);
...
export function initAgentReputation(deps) {
  ...
  async function tokenIdFor(userId, address) {
    ...
    // Bounded backward Transfer-log scan (mints land here when identities
    // are fresh; older ones rely on the persisted table above).
    let toBlock = await publicClient.getBlockNumber();
    for (let i = 0; i < SCAN_CHUNKS_MAX && toBlock > 0n; i++) {
      const fromBlock = toBlock > SCAN_CHUNK ? toBlock - SCAN_CHUNK : 0n;
      try {
        const logs = await publicClient.getLogs({
          address: IDENTITY_REGISTRY, event: TRANSFER_EVENT,
          args: { to: address }, fromBlock, toBlock,
        });
        if (logs.length > 0) {
          const id = String(logs[logs.length - 1].args.tokenId);
      ...
      } catch (_) { /* range errors etc. — keep walking back */ }
      if (fromBlock === 0n) break;
      toBlock = fromBlock - 1n;
    }
    tokenIdCache.set(address, null);
    return null;
  }
