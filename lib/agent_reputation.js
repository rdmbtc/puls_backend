...
/**
 * Aggregate peer-review activity per agent from peer_review marker rows
 * (notifications type='peer_review', message = JSON marker). Pure; the route
 * feeds it everything it selected. No chain scans.
 *
 * @param {Array<{title:string, message:string}>} rows — durable markers
 * @returns {Map<string,{agent,reviewsGiven,reviewsReceived,scoresReceived:number[],avgScore}>}
 *   keyed by canonical agent id (userId or `agent_stack_<raterKey>` fallback)
 */
export function aggregateReputations(rows) {
  const agents = new Map();
  const ensure = (id) => {
    let a = agents.get(id);
    if (!a) {
      a = { agent: id, reviewsGiven: 0, reviewsReceived: 0, scoresReceived: [], avgScore: null };
      agents.set(id, a);
    }
    return a;
  };
  for (const row of (rows || [])) {
    let m;
    try { m = JSON.parse(row.message || '{}'); } catch { continue; }
    if (!m || typeof m !== 'object') continue;
    const raterId = m.raterKey ? (candidateUserIds(m.raterKey)[0]) : `agent_stack_${row.title?.split('|')[0]}`;
    const targetId = m.targetUserId;
    if (!targetId && !m.skipped) continue;

    if (m.txHash) {
      // A posted on-chain review: counts as GIVEN by rater, RECEIVED by target.
      if (m.raterKey) {
        const g = ensure(raterId);
        g.reviewsGiven += 1;
      }
      if (targetId) {
        const t = ensure(targetId);
        t.reviewsReceived += 1;
        if (Number.isFinite(Number(m.score))) t.scoresReceived.push(Number(m.score));
      }
    }
  }
  for (const a of agents.values()) {
    a.avgScore = a.scoresReceived.length
      ? +(a.scoresReceived.reduce((s, v) => s + v, 0) / a.scoresReceived.length).toFixed(1)
      : null;
    delete a.scoresReceived;
  }
  return agents;
}

/** Marker rows → sorted aggregate array for the API. */
export function reputationsToList(rows) {
  return [...aggregateReputations(rows).values()].sort(
    (a, b) => b.reviewsGiven + b.reviewsReceived - (a.reviewsGiven + a.reviewsReceived)
  );
}
...
export function initAgentReputation(deps) {
  const {
    supabase, publicClient, keccak256, toHex, app,
  ...
  async function alreadyReviewed(key) {
    ...
    try {
      // Column is `message` on some schemas, `body` on others — select * and
  ...
  /** Insert the durable marker; tolerate schemas without a `message` column. */
  async function insertMarker(row) {
    const first = await supabase.from('notifications').insert(row);
    if (first?.error && /message/i.test(first.error.message || '')) {
      const { message: _m, ...rest } = row;
      const second = await supabase.from('notifications').insert({ ...rest, body: _m });
  ...
  // ── Read side: per-agent review activity (no chain scans) ───────────────────
  if (app) {
    app.get('/api/agents/reputation', async (_req, res) => {
      try {
        ...
        // Normalize the marker payload column (message vs body per schema).
        const normalized = (rows || []).map((r) => ({
          title: r.title,
          message: r.message ?? r.body ?? '{}',
        }));
        const agents = reputationsToList(normalized);
        const totalReviews = agents.reduce((s, a) => s + a.reviewsGiven, 0);
        res.json({
          ok: true,
          ...
          count: agents.length,
          agents,
          note: 'Aggregated from posted on-chain peer reviews (ERC-8004 giveFeedback via Agent Wallets). Duel outcomes score 90 (win) / 40 (loss); signal purchases carry the buyer post-trade confidence clamped 10..95.',
        });
      } catch (e) {
        console.error('[peer-rep] reputation read error:', e.message);
        res.status(500).json({ error: e.message });
      }
    });
    console.log('[peer-rep] register: GET /api/agents/reputation');
  }
