 ...
 *
 * Sentry v8: uses `expressErrorHandler()` (was `Handlers.errorHandler()` in v7).
 */
export const sentryErrorHandler = Sentry.expressErrorHandler();

/**
 * Sentry request handler — in v8, the SDK auto-instruments Express when
 * init() is called, so there's no separate request-handler middleware to
 * mount. This is a no-op for compatibility with the v7 wiring pattern.
 */
