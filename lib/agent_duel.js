...
export function registerAgentDuel(app, deps) {
  ...
  // counterparties on-chain after settlement (wired from this module — no
  // server.js changes).
  const reputation = initAgentReputation({ supabase, publicClient, keccak256, toHex, getWalletId, getWalletInfo });
    ...
    try {
      ...
      const [{ data: duelRows }, { data: bondRows }] = await Promise.all([
        supabase.from('agent_duels').select('open_tx, join_tx, opened_at, joined_at')
          .gte('opened_at', new Date(now - 7 * 24 * 3600_000).toISOString()).limit(500),
        supabase.from('creator_signals').select('bond_post_tx, bond_posted_at')
          .not('bond_post_tx', 'is', null)
          .gte('bond_posted_at', new Date(now - 7 * 24 * 3600_000).toISOString()).limit(500),
      ]);
      for (const r of (duelRows || [])) {
        if (isUuid(r.open_tx) && r.opened_at && r.opened_at < cutoff) stuck++;
        if (isUuid(r.join_tx) && r.joined_at && r.joined_at < cutoff) stuck++;
      }
      for (const r of (bondRows || [])) {
        if (isUuid(r.bond_post_tx) && r.bond_posted_at && r.bond_posted_at < cutoff) stuck++;
      }
