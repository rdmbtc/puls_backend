...
/**
 * Decode a Circle CLI session and compute how long it is still valid.
 * Accepts the base64 CIRCLE_AGENT_SESSION_B64 value, a raw JSON string, or an
 * already-parsed object. Reads `<profile>.expiresAt` (epoch seconds or ms).
 *
 * @param {string|object} [input]  defaults to the CIRCLE_AGENT_SESSION_B64 env var
 * @param {number} [now]           epoch ms override (tests)
 * @returns {{ expiresAtMs:number, daysLeft:number }|null} null when absent/unparseable
 */
export function parseSessionExpiry(input, now = Date.now()) {
  try {
    let obj = input ?? process.env.CIRCLE_AGENT_SESSION_B64;
    if (obj == null) return null;
    if (typeof obj === 'string') {
      const raw = obj.trim();
      obj = JSON.parse(raw.startsWith('{') ? raw : Buffer.from(raw, 'base64').toString('utf-8'));
    }
    if (typeof obj !== 'object' || obj == null) return null;
    // The agent profile nests under its profile name ("testnet"); be lenient.
    let exp = obj?.testnet?.expiresAt ?? obj?.mainnet?.expiresAt ?? obj?.expiresAt;
    exp = Number(exp);
    if (!Number.isFinite(exp) || exp <= 0) return null;
    if (exp < 1e12) exp *= 1000; // epoch seconds → ms
    return { expiresAtMs: exp, daysLeft: (exp - now) / 86_400_000 };
  } catch {
    return null;
  }
}
...
// ── API-lockout backoff ──────────────────────────────────────────────────────
// Circle answers 504 "Lockout" pages when an account hammers the API with
// failing requests (e.g. during a chain-side execution incident). While
// locked out, every extra call extends the pain — so all wrapper calls share
// one account-wide cooldown.
let _lockoutUntil = 0;
const LOCKOUT_MS = () => parseInt(process.env.CIRCLE_AGENT_LOCKOUT_MS || '300000', 10); // 5 min

function isLockoutError(msg) {
  const m = String(msg || '');
  // Per-wallet nonce-queue rejections are NOT an account lockout.
  if (/wait for pending/i.test(m)) return false;
  return /\blockout\b|\b50[34]\b|\b429\b|too many requests/i.test(m);
}
...
export async function executeContract({ signature, params = [], contract, address }) {
  if (Date.now() < _lockoutUntil) {
    throw new Error(`circle API lockout cooldown active (${Math.ceil((_lockoutUntil - Date.now()) / 1000)}s left)`);
  }
  ensureSession();
  ...
  let parsed;
  try {
    parsed = await execCliJson([
      'wallet', 'execute', signature, ...params.map(String),
      '--contract', contract, '--address', address, '--chain', CHAIN,
    ], { timeoutMs: 200_000 });
  } catch (e) {
    // The CLI polls the signing challenge for only ~60s before giving up.
    // When Circle's signing queue is slow the challenge still completes
    // server-side and the transaction executes minutes later (observed
    // 2026-08-22: INITIATED txs drained to COMPLETE hours afterwards).
    // So a CLI timeout ≠ failure: locate the freshly created transaction
    // and keep polling it ourselves.
    if (!isLockoutError(e.message)) {
      const late = await waitForNewestTransaction(address, { maxWaitMs: 480_000 });
      if (late && String(late.state || '').toUpperCase() === 'COMPLETE') {
        return { id: late.id, state: 'COMPLETE', txHash: late.txHash || null };
      }
    }
    if (isLockoutError(e.message)) _lockoutUntil = Date.now() + LOCKOUT_MS();
    throw e;
  }
...
export async function payService(url, address) {
  ...
  let parsed;
  try {
    parsed = await execCliJson(['services', 'pay', url, '--address', address, '--chain', CHAIN]);
  } catch (e) {
    if (isLockoutError(e.message)) _lockoutUntil = Date.now() + LOCKOUT_MS();
    throw e;
  }
...
/**
 * Poll the wallet's newest transaction until it reaches a terminal state.
 * Used to outlive the CLI's short (~60s) signing-challenge patience: slow
 * challenges complete server-side and the tx lands minutes later.
 */
export async function waitForNewestTransaction(address, { maxWaitMs = 480_000, pollMs = 15_000 } = {}) {
  ensureSession();
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (txs.length > 0) {
        if (!newest) newest = txs[0];
        const cur = txs.find((t) => t.id === newest.id) || txs[0];
        const st = String(cur.state || '').toUpperCase();
        if (['COMPLETE', 'CONFIRMED'].includes(st)) return cur;
        if (['FAILED', 'CANCELLED', 'DENIED'].includes(st)) return cur;
      }
    } catch { /* transient API errors — keep polling */ }
...
/**
 * Pending-transaction summary for a wallet: how many INITIATED txs Circle
 * holds and how old the OLDEST one is (ms). Powers the stuck-tx watchdog.
 */
export async function pendingSummary(address) {
  ensureSession();
  if (!address) return { count: 0, oldestAgeMs: 0 };
  try {
    const parsed = await execCliJson(['transaction', 'list', '--address', address, '--chain', CHAIN]);
    const now = Date.now();
    let count = 0, oldest = 0;
    for (const t of parsed?.data?.transactions || []) {
      if (String(t.state || '').toUpperCase() !== 'INITIATED') continue;
      count++;
      const created = Date.parse(t.createDate || '');
      if (Number.isFinite(created)) oldest = Math.max(oldest, now - created);
    }
    return { count, oldestAgeMs: oldest };
  } catch {
    return { count: -1, oldestAgeMs: 0 };
  }
}
...
export async function buyMarketsSnapshot(address, { minIntervalMs = 30 * 60 * 1000, quiet = false } = {}) {
  // Failure also starts a cooldown: a rejected payment may still have moved
  // funds ("payment submitted"), so never hammer the endpoint every cycle.
  ...
  try {
    ...
    _lastSnap = { ts: Date.now(), receipt: null };
    console.warn(`[agent-wallet] buyMarketsSnapshot failed (cooldown ${Math.round(minIntervalMs / 60000)}min): ${e.message}`);
    return null;
  }
}
...
export default {
  ...
  ensureSession, executeContract, payService, usdcBalance, buyMarketsSnapshot,
  parseSessionExpiry, status,
};
