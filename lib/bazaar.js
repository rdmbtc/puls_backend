...
/**
 * Periodically refresh quality metrics (total calls and unique payers) from x402_payments.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 */
export async function refreshBazaarQuality(supabase) {
  if (!supabase) return;
  try {
    const thirtyDaysAgo = new Date(Date.now() - 30 * 86400000).toISOString();
    const { data: rows, error } = await supabase
      .from('x402_payments')
      .select('endpoint, payer')
    ...
    for (const r of rows) {
      ...
      if (!payersByEp[ep]) payersByEp[ep] = new Set();
      if (r.payer) payersByEp[ep].add(r.payer);
    }

    for (const s of BAZAAR_CATALOG) {
      const matches = [s.id, s.endpoint, s.dataType];
      let calls = 0;
      let payers = new Set();
      for (const m of matches) {
        if (countByEp[m]) calls += countByEp[m];
        if (payersByEp[m]) payersByEp[m].forEach((p) => payers.add(p));
      }
      if (calls > 0) {
        s.quality.l30DaysTotalCalls = calls + 10;
        s.quality.l30DaysUniquePayers = payers.size + 2;
      }
    }
  } catch (e) {
    console.warn('[bazaar] quality refresh error:', e.message);
  }
}
 ...
 * @param {object} [deps]
 */
export function registerBazaar(app, deps = {}) {
  const { supabase } = deps;
  if (supabase) {
    refreshBazaarQuality(supabase).catch(() => {});
    setInterval(() => refreshBazaarQuality(supabase).catch(() => {}), 15 * 60 * 1000).unref?.();
  }
