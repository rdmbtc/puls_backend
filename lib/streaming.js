...
export function registerStreaming(app, deps) {
  ...
  if (STREAMS_ENABLED) {
    // Boot sweep catches anything stale from a server crash; afterwards the
    // timer is event-driven (kicked by openStream, stops when no active streams).
    setTimeout(() => reconcile().then((n) => { if (n > 0) scheduleReconcile(); }), 30_000).unref?.();
  }
  ...
  // Public live streams endpoint — shows all active/paused streams (no auth).
  // For judges/visitors to see pay-per-second streaming in action.
  ...
  // History endpoint — last 10 completed streams.
  ...
  return { openStream, tick: tickStream, pause: pauseStream, resume: resumeStream, stop: stopStream, settle: settleStream, get: getStream, reconcile, scheduleReconcile };
}
