...
export function registerStreaming(app, deps) {
  ...
  async function openStream({ payerUserId, recipientUserId, recipientAddress, resource, ratePerSecUsdc, capUsdc, split, openedBy = 'user', meta = {} }) {
    ...
    // Kick the event-driven reconcile timer so this stream's staleness + settlement
    // are watched. The timer self-disarms when no active streams remain.
    scheduleReconcile();
    return data;
  }
  ...
  // Event-driven: the timer only runs while there are active streams. When
  // reconcile finds no active/paused streams, it stops re-arming → zero
  // polling when idle. openStream kicks the timer; stopStream lets it expire.
  let reconciling = false;
  let _reconcileTimer = null;
  function scheduleReconcile() {
    if (_reconcileTimer) return; // already armed
    _reconcileTimer = setTimeout(async () => {
      _reconcileTimer = null;
      const activeCount = await reconcile();
      // Re-arm only if there are still active/paused streams to watch.
      if (activeCount > 0) scheduleReconcile();
    }, RECONCILE_SEC * 1000);
  ...
  async function reconcile() {
    ...
    try {
      ...
      if (error) return 0; // table may not exist yet (migration pending) — stay quiet
      for (const s of rows || []) {
      ...
      return (rows || []).length; // >0 → scheduleReconcile re-arms; 0 → timer stops
    } catch (e) {
  ...
  // Public live streams endpoint — must be BEFORE /api/streams/:id to avoid
  // being caught by the :id parameter matcher.
  app.get('/api/streams/live', async (_req, res) => {
    try {
      ...
      const streams = (data || []).map((s) => ({
        id: s.id,
        payer: nameFor(s.payer_user_id),
        payerAddress: s.payer_address,
        recipient: nameFor(s.recipient_user_id),
        recipientAddress: s.recipient_address,
        resource: s.resource,
        ratePerSecUsdc: Number(s.rate_per_sec_usdc),
        capUsdc: Number(s.cap_usdc),
        status: s.status,
        accruedUsdc: Number(s.accrued_micro) / 1e6,
        settledUsdc: Number(s.settled_micro) / 1e6,
        startedAt: s.started_at,
        lastTickAt: s.last_tick_at,
      }));
      res.json({ streams, count: streams.length, live: STREAMS_ENABLED });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });

  // History endpoint — last 10 completed streams. Also before :id.
  app.get('/api/streams/history', async (_req, res) => {
    try {
      const { data, error } = await supabase
        .from('payment_streams').select('*')
        .eq('status', 'stopped')
      ...
      const streams = (data || []).map((s) => ({
        ...
        ratePerSecUsdc: Number(s.rate_per_sec_usdc),
        capUsdc: Number(s.cap_usdc),
        totalAccruedUsdc: Number(s.accrued_micro) / 1e6,
        totalSettledUsdc: Number(s.settled_micro) / 1e6,
        settleTx: s.settle_tx ? `https://testnet.arcscan.app/tx/${s.settle_tx}` : null,
        startedAt: s.started_at,
        stoppedAt: s.stopped_at,
      }));
  ...
  if (STREAMS_ENABLED) {
    // Boot sweep catches anything active/stale from a previous process restart; afterwards the
    // timer is event-driven (kicked by openStream, stops when no active streams).
    setTimeout(() => reconcile().then((n) => { if (n > 0) scheduleReconcile(); }), 1000).unref?.();
  }
  ...
  return { openStream, tick: tickStream, pause: pauseStream, resume: resumeStream, stop: stopStream, settle: settleStream, get: getStream, reconcile, scheduleReconcile, flushOnShutdown: reconcile };
}
