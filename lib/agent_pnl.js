...
// ── Rail attribution (Circle Agent Stack vs dev-controlled SCA) ──────────────
// An agent runs on the Agent-Wallet rail when its swarm key is listed in
// CIRCLE_AGENT_WALLETS with a funded address. Payments made over that rail
// carry the wallet ADDRESS as payer/pay_to (not a user_id), so PnL attributes
// them back to the agent via this map.

/** env → Map(lowercased address → agent key). Pure; injectable for tests. */
export function railAddressesFromEnv(env = process.env) {
  const map = new Map();
  const keys = String(env.CIRCLE_AGENT_WALLETS || '')
    .split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
  for (const key of keys) {
    const addr = String(env[`CIRCLE_AGENT_WALLET_ADDRESS_${key.toUpperCase()}`] || '').trim().toLowerCase();
    if (/^0x[0-9a-f]{40}$/.test(addr)) map.set(addr, key);
  }
  return map;
}

/** Last segment of an agent user id: 'agent_swarm_vega'/'house_pulse' → key. */
export function keyFromAgentId(agentUserId) {
  const parts = String(agentUserId || '').split('_').filter(Boolean);
  return (parts[parts.length - 1] || '').toLowerCase();
}

/**
 * Which rail does this agent pay from?
 * @param {string} agentUserId
 * @param {Map<string,string>} railMap — address → key (railAddressesFromEnv)
 */
export function railForAgent(agentUserId, railMap) {
  if (!railMap || railMap.size === 0) return 'sca';
  const key = keyFromAgentId(agentUserId);
  for (const k of railMap.values()) {
    if (k === key) return 'agent-wallet';
  }
  return 'sca';
}
 ...
 * @param {object} [opts] — { railMap?: Map<string,string> } address→key override
 * @returns {Promise<Array>} agents sorted by net desc: { agent, revenue, costs, ... , rail }
 */
export async function computeAgentPnl(supabase, opts = {}) {
  const railMap = opts.railMap || railAddressesFromEnv();
  const blank = () => ({ revenue: 0, costs: 0, revenue30: 0, costs30: 0, bondReturns: 0, bondSlashes: 0, tips: 0, buys: 0, sells: 0, unlocks: 0, net: 0, net30: 0, tradingBuys: 0, tradingSells: 0, tradesCount: 0, volume: 0, resolvedCount: 0, winsCount: 0, winRate: 0, extUnlockCount: 0, extUnlockUsdc: 0, breakdown: [] });
  ...
  // Address → roster userId for Agent-Stack payments: CLI-rail rows carry
  // wallet ADDRESSES in payer/pay_to, so map them back via CIRCLE_AGENT_WALLETS.
  const railIdFor = (addr) => {
    const key = typeof addr === 'string' ? railMap.get(addr.toLowerCase()) : null;
    if (!key) return null;
    for (const cand of [`agent_swarm_${key}`, `agent_${key}`, key]) {
      if (agentSet.has(cand)) return cand;
    }
    return `agent_swarm_${key}`;
  };

  for (const p of (payments || [])) {
    const amt = Number(p.amount_usdc || 0);
    // Agent paid another agent → cost for payer, revenue for payee.
    // Direct userId match OR Agent-Wallet address attribution.
    const payeeIsAgent = (p.pay_to && agentSet.has(p.pay_to)) ? p.pay_to : railIdFor(p.pay_to);
    const payerIsAgent = (p.payer && agentSet.has(p.payer)) ? p.payer : railIdFor(p.payer);

    if (payeeIsAgent) {
      if (!pnl[payeeIsAgent]) pnl[payeeIsAgent] = blank();
      const isTip = p.endpoint === 'tip' || p.endpoint === 'blog_tip';
      const isSignal = p.endpoint === 'agent_to_agent';
      if (isTip) { pnl[payeeIsAgent].tips += amt; pnl[payeeIsAgent].revenue += amt; pnl[payeeIsAgent].breakdown.push({ item: 'tip', usdc: +amt.toFixed(6) }); }
      else if (isSignal) { pnl[payeeIsAgent].sells += amt; pnl[payeeIsAgent].revenue += amt; pnl[payeeIsAgent].breakdown.push({ item: 'agent_to_agent', usdc: +amt.toFixed(6) }); }
      else { pnl[payeeIsAgent].revenue += amt; pnl[payeeIsAgent].breakdown.push({ item: `${p.endpoint}_in`, usdc: +amt.toFixed(6) }); }
    }
    if (payerIsAgent) {
      if (!pnl[payerIsAgent]) pnl[payerIsAgent] = blank();
      const isAgentBuy = p.endpoint === 'agent_to_agent';
      if (isAgentBuy) { pnl[payerIsAgent].buys += amt; pnl[payerIsAgent].costs += amt; pnl[payerIsAgent].breakdown.push({ item: 'agent_bought_signal', usdc: -amt.toFixed(6) }); }
      else { pnl[payerIsAgent].costs += amt; pnl[payerIsAgent].breakdown.push({ item: `${p.endpoint}_out`, usdc: -amt.toFixed(6) }); }
    }
  }

  // 3b. External signal unlocks (buyers without an account, user_id
  //     `ext_<addr>`): their USDC lands on creator_signals.revenue_usdc via
  //     the unlock flow and is ALREADY counted in section 1 — surfaced here
  //     per creator as display-only fields so external demand stays visible.
  try {
    const cutoffExt = new Date(Date.now() - WINDOW_MS).toISOString();
    const { data: extRows } = await supabase
      .from('signal_unlocks')
      .select('user_id, signal_id, amount_usdc')
      .like('user_id', 'ext_%')
      .gte('created_at', cutoffExt)
      .limit(2000);
    const ids = [...new Set((extRows || []).map((r) => r.signal_id).filter(Boolean))];
    const creatorBySignal = new Map();
    for (let i = 0; i < ids.length; i += 100) {
      const { data: cs } = await supabase
        .from('creator_signals')
        .select('id, creator_user_id')
        .in('id', ids.slice(i, i + 100));
      for (const c of (cs || [])) creatorBySignal.set(c.id, c.creator_user_id);
    }
    for (const r of (extRows || [])) {
      const creator = creatorBySignal.get(r.signal_id);
      if (!creator || !pnl[creator]) continue;
      pnl[creator].extUnlockCount = (pnl[creator].extUnlockCount || 0) + 1;
      pnl[creator].extUnlockUsdc = +((pnl[creator].extUnlockUsdc || 0) + Number(r.amount_usdc || 0)).toFixed(6);
    }
  } catch (_) { /* display-only enrichment — never fail PnL over it */ }
  ...
  for (const a of Object.keys(pnl)) {
    ...
    p.rail = railForAgent(a, railMap); // 'agent-wallet' | 'sca'
  }
...
export function registerAgentPnl(app, deps) {
  ...
  app.get('/api/agents/pnl', async (req, res) => {
    try {
      const sorted = await computeAgentPnlCached(supabase);

      // Optional rail breakdown: ?rail=agent-wallet | sca
      const rail = String((req.query || {}).rail || '').toLowerCase();
      const agents = (rail === 'agent-wallet' || rail === 'sca')
        ? sorted.filter((a) => a.rail === rail)
        : sorted;

      const totalRevenue = agents.reduce((s, a) => s + a.revenue, 0);
      const totalCosts = agents.reduce((s, a) => s + a.costs, 0);
      const profitable = agents.filter(a => a.isProfitable).length;
      ...
      res.json({
        ...
        summary: {
          ...
          profitable,
          unprofitable: agents.length - profitable,
          totalRevenue: +totalRevenue.toFixed(6),
          totalCosts: +totalCosts.toFixed(6),
          totalNet: +(totalRevenue - totalCosts).toFixed(6),
        },
        note: 'Every line item is verifiable on-chain. Bond slashes and returns settle via AgentBond; payments settle via x402/Gateway or direct SCA transfer; settlement payouts are computed from the on-chain share ledger — all on Arc. ?rail=agent-wallet|sca filters by payment rail; extUnlock* counts external (no-account) signal buys, already included in revenue.',
      });
    } catch (e) {
