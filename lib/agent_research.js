...
/**
 * Clean stop-words, boilerplate phrasing, and trailing expiration clauses
 * from prediction market questions to generate crisp, high-relevance search queries.
 */
export function cleanMarketQuestionForSearch(question) {
  if (!question || typeof question !== 'string') return '';
  let q = question.trim();
  // Strip prediction market prefixes: "Will ", "Is ", "Does ", "Can ", "Who will ", etc.
  q = q.replace(/^(will|is|does|can|who will|which|what)\s+(the\s+)?/i, '');
  // Strip trailing resolution clauses / dates that confuse search engines:
  q = q.replace(/\s+(before|by|on|prior to|after|in)\s+(the\s+end\s+of\s+)?(january|february|march|april|may|june|july|august|september|october|november|december|\d{1,2}(st|nd|rd|th)?|\d{4})\b.*$/i, '');
  // Strip question marks and trailing punctuation
  q = q.replace(/[?.,!;:"]+$/g, '').trim();
  // Fallback if over-stripped
  if (q.length < 4) q = question.replace(/[?"]/g, '').trim();
  return q;
}
...
export async function webSearch(query, limit = 8) {
  ...
  const searchQuery = cleanMarketQuestionForSearch(query) || query;
  ...
  if (EXA_API_KEY) {
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data.results) && data.results.length > 0) {
          const results = data.results.map((r) => {
            ...
            const highlightText = Array.isArray(r.highlights) ? r.highlights.join(' ... ') : '';
            return {
              title: r.title || 'News Update',
              url: r.url,
              snippet: highlightText || (r.text ? r.text.slice(0, 300) : ''),
              source,
              rawContent: r.text || highlightText,
      ...
      console.warn('[research] Exa search failed, falling back:', e.message);
    }
  }
  ...
  try {
    const target = `${DDG_LITE}?q=${encodeURIComponent(searchQuery)}`;
...
async function fetchComposioSocial(question) {
  ...
  try {
    ...
    // CRITICAL: Wrap each in a Promise.race timeout — a hung Composio call
    // previously blocked Promise.all indefinitely, stalling agent ticks.
    const withTimeout = (promise, ms) =>
      Promise.race([promise, new Promise(r => setTimeout(() => r(null), ms))]);

    const tasks = [];
    tasks.push(withTimeout(composioClient.tools.execute('TWITTER_SEARCH_RECENT', { query: keywords }, 'default').catch(() => null), 10_000));
    tasks.push(withTimeout(composioClient.tools.execute('REDDIT_SEARCH', { query: keywords }, 'default').catch(() => null), 10_000));
    tasks.push(withTimeout(composioClient.tools.execute('GITHUB_SEARCH_ISSUES', { query: keywords }, 'default').catch(() => null), 10_000));
...
export async function researchQuestion(question, limit = 8) {
  ...
  if (provider === 'exa') {
    // Exa Neural Search provides query-relevant highlights and page excerpts
    brief += hits
      .map((h, i) => {
        const body = h.rawContent ? extractRelevant(h.rawContent.slice(0, 4000), question, 1800) : h.snippet;
        const dateTag = h.publishedDate ? ` · ${h.publishedDate.slice(0, 10)}` : '';
        return `[${i + 1}] ${h.title} (${h.source}${dateTag})\n${body || h.snippet}`;
      })
      .join('\n\n');
