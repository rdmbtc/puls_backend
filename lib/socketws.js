...
export function initRawWs(server) {
  ...
  // WebSocket ping/pong keepalive + application heartbeat — prevents H15 "Idle connection" errors.
  // Heroku drops connections idle for >55s. Protocol ping + JSON heartbeat frame keep the proxy active.
  const HEARTBEAT_FRAME = JSON.stringify({ type: 'ping', ts: 0 });
