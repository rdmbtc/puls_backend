...
// the contract returns/slashes the bond itself and needs no poster signature.
// Bond stakes (approve/postBond) may originate from EITHER rail, decided by
// chooseRail() in lib/agent_rail.js:
...
export function registerAgentBond(app, deps) {
  ...
  // Resolve a bond poster on whichever rail holds its funds (decided by
  // chooseRail): Agent Wallet → CLI balance; dev-controlled SCA → getWalletInfo.
  // Null = no usable wallet.
  ...
  async function postPass() {
    const rows = await (async () => {
      ...
      // bond_status='active' but no on-chain post and no bond_posted_at yet).
      // Give ENABLED Agent-Stack creators a real bond within the settlePass
      // phantom-void window; other creators keep the legacy behavior.
  ...
  //
  // Plus a periodic sweep (AGENT_BOND_INTERVAL_MIN, default 5min): swarm
  // creators insert signals directly and don't emit SIGNAL_PUBLISHED, so
  // without this their optimistic 'active' rows would only be bonded when an
  // unrelated event happened to fire.
  setTimeout(reconcile, 60_000);
  setInterval(() => {
    reconcile().catch((e) => console.warn('[agent_bond] periodic reconcile:', e.message));
  }, INTERVAL_MIN * 60 * 1000).unref?.();
