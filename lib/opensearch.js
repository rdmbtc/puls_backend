...
if (OPENSEARCH_URL) {
  try {
    // TLS policy: STRICT verification by default (Aiven managed services now
    // present publicly-trusted chains, e.g. anchored at ISRG Root X1). Pin a
    // custom CA via OPENSEARCH_CA_CERT, or set OPENSEARCH_INSECURE_TLS=true
    // for self-signed endpoints that can't be pinned.
    const caPath = process.env.OPENSEARCH_CA_CERT;
    const insecure = String(process.env.OPENSEARCH_INSECURE_TLS || '').toLowerCase() === 'true';
    const ssl = caPath
