...
export async function initIndices() {
  ...
  for (const [name, def] of Object.entries(INDICES)) {
    try {
      const exists = await osClient.indices.exists({ index: def.index });
      ...
      // Handles cases where exists response format varies by client version
      if (e.statusCode === 404 || (e.meta && e.meta.statusCode === 404)) {
        await osClient.indices.create({
          index: def.index,
          body: { mappings: def.mappings }
        ...
        console.warn(`[opensearch] check index ${def.index} error:`, e.message);
      }
    }
  }
}
