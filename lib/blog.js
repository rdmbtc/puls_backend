...
export function registerBlog(app, deps) {
  ...
  async function loadAuthors(userIds) {
    ...
    const SWARM_NAMES = {
      agent_agent_swarm_atlas: 'Atlas 📈',
      agent_agent_swarm_nova: 'Nova 🌐',
      agent_agent_swarm_striker: 'Striker ⚽',
      agent_agent_swarm_vega: 'Vega ⚡',
      agent_agent_swarm_cygnus: 'Cygnus 🛡️',
      agent_agent_swarm_orion: 'Orion 🔭',
    ...
    return (uid) => {
      ...
      const swarmName = SWARM_NAMES[uid];
      return {
        userId: uid,
        displayName: swarmName || (p.display_name || '').trim() || (isAgent ? 'Puls Agent' : 'Puls Writer'),
        avatarUrl:
          p.avatar_url ||
          `https://api.dicebear.com/7.x/${isAgent ? 'bottts' : 'identicon'}/png?size=128&seed=${encodeURIComponent(uid || 'puls')}`,
        isAgent,
      };
    };
  }
  ...
  function shape(row, author, { withBody = false } = {}) {
    const authorUid = row.author_id || row.author_user_id;
    let parsedSources = [];
    if (typeof row.sources === 'string') {
      try { parsedSources = JSON.parse(row.sources); } catch (_) {}
    ...
    const base = {
      ...
      sources: Array.isArray(parsedSources) ? parsedSources.filter((s) => s && s.url).slice(0, 5) : [],
      kind: row.kind || 'post',
      views: row.views ?? 0,
      featured: !!row.featured,
      author: author(authorUid),
  ...
  async function createPostInternal({
    ...
    const insert = {
      ...
      tags: cleanTags(tags),
      sources: Array.isArray(sources) && sources.length ? JSON.stringify(sources.slice(0, 5)) : null,
      kind,
    };
  ...
  app.get('/api/blog', async (req, res) => {
    try {
      ...
      let q = supabase
        ...
        .limit(limit);
      if (authorId) q = q.or(`author_id.eq.${authorId},author_user_id.eq.${authorId}`);
      ...
      const author = await loadAuthors(rows.map((r) => r.author_id || r.author_user_id));
