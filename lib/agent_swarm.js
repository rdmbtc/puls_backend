...
export function registerSwarm(app, deps) {
  ...
  async function ensureAgent(cfg) {
    ...
    if (!s.registered) {
      ...
      if (!existing && agentHasIdentity && await agentHasIdentity(info.address)) existing = true; // already owns one — don't re-mint
      if (existing) {
        s.registered = true;
      } else if (balance >= 0.2) {
        try {
  ...
  app.get('/api/agents/roster', async (req, res) => {
    try {
      ...
      for (const cfg of ROSTER) {
        ...
        // ERC-8004 id: cache → resolve (persisted store / bounded scan) on miss,
        // so the id shows right after a restart instead of waiting for the agent's
        // next trade tick. resolveAgentTokenId caches + persists, so the cost is
        // one-time per cold cache.
    ...
    try {
      ...
      const events = (rows || []).map(r => {
        ...
        return {
          ...
          // pay-per-second streaming (RFB 4): rate, how much flowed/settled, why it stopped
