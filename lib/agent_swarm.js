...
{"slug":"...","side":"YES"|"NO","confidence":<0-100 integer>,"reasoning":"<2-3 sentences in your voice. MUST include: (1) cite the consensus probability, (2) cite at least one specific web finding with source name, (3) explain your edge. Be concrete, not generic.>","counterargument":"<1 sentence: what's the strongest reason you could be WRONG?>","riskNote":"<1 sentence: if you're wrong, you lose the stake — how much USDC and what's the downside?>"}`;
  ...
  async function runCreator(cfg) {
    ...
    if (error) {
      console.error(`[swarm:${cfg.key}] signal insert FAIL:`, error.message);
      // Full error object for PostgREST-level diagnostics
      try {
        const errDetail = {
        ...
        // Log the body keys and types to pinpoint which column fails
        const bodySummary = {};
        for (const [k, v] of Object.entries(body)) {
          bodySummary[k] = { t: typeof v };
          if (typeof v === 'string') {
            bodySummary[k].len = v.length;
            // Check for lone surrogates directly
            let loneHigh = 0, loneLow = 0;
            for (let i = 0; i < v.length; i++) {
              const c = v.charCodeAt(i);
              if (c >= 0xD800 && c <= 0xDBFF) {
                if (i + 1 >= v.length || v.charCodeAt(i + 1) < 0xDC00 || v.charCodeAt(i + 1) > 0xDFFF) loneHigh++;
              }
              if (c >= 0xDC00 && c <= 0xDFFF) {
                if (i === 0 || v.charCodeAt(i - 1) < 0xD800 || v.charCodeAt(i - 1) > 0xDBFF) loneLow++;
              }
            }
            bodySummary[k].loneHighSurrogates = loneHigh;
            bodySummary[k].loneLowSurrogates = loneLow;
            bodySummary[k].codes = [...v.slice(0, 80)].map(c => c.charCodeAt(0));
            bodySummary[k].sample = JSON.stringify(v.slice(0, 100));
          } else if (Array.isArray(v)) {
            bodySummary[k].arrLen = v.length;
            bodySummary[k].sample = JSON.stringify(v).slice(0, 300);
          } else {
            bodySummary[k].val = JSON.stringify(v)?.slice(0, 100);
          }
        }
        console.error(`[swarm:${cfg.key}] DEBUG signal body:`, JSON.stringify(bodySummary));
      } catch (e) { console.error(`[swarm:${cfg.key}] DEBUG error:`, e.message); }
      return;
    }
  ...
  function start() {
    ...
    setTimeout(() => {
      console.log('[swarm] Bootstrapping initial colony activity & autonomous creator signals...');
      creators.forEach((cfg, i) => setTimeout(() => runCreator(cfg).catch((err) => console.error('[swarm] init creator signal err:', err.message)), i * 3000));
    ...
    setInterval(() => {
      creators.forEach((cfg, i) => setTimeout(() => runCreator(cfg).catch(() => {}), i * 5000));
    }, 15 * 60 * 1000).unref?.();
