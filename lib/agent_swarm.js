...
{"slug":"...","side":"YES"|"NO","confidence":<0-100 integer>,"reasoning":"<2-3 sentences in your voice. MUST include: (1) cite the consensus probability, (2) cite at least one specific web finding with source name, (3) explain your edge. Be concrete, not generic.>","counterargument":"<1 sentence: what's the strongest reason you could be WRONG?>","riskNote":"<1 sentence: if you're wrong, you lose the stake — how much USDC and what's the downside?>"}`;
  ...
  async function runCreator(cfg) {
    ...
    //   - C0 control chars / NULL (\u0000-\u001F minus \t\n\r) and \u007F
    //   - UNPAIRED UTF-16 surrogates (\uD800-\uDFFF alone) — JSON.stringify
    //     emits them as literal \uD800 etc., which Postgres' JSON parser
    //     rejects. LLM output (Gemini/DeepSeek error states) occasionally
    //     produces these. This silently dropped every creator signal.
    const stripCtrl = (v) => {
    ...
    const cleanSources = (researchSources || []).map((s) => ({
      title: stripCtrl(s?.title), url: stripCtrl(s?.url), source: stripCtrl(s?.source),
    }));
    const body = {
      creator_user_id: cfg.user, title: stripCtrl(pick.t), market_question: stripCtrl(pick.q), stance: pick.s,
      market_slug: pick.marketSlug || null,
      confidence: pick.c, edge_bps: pick.e, horizon: pick.h,
      teaser: stripCtrl(`${cfg.name} has a ${cleanSources.length}-source researched call on "${pick.t}". Unlock to see the side + the full thesis.`),
      thesis: stripCtrl(thesis), price_usdc: 0.001, status: 'published', published_at: new Date().toISOString(),
    ...
    if (error) {
      console.error(`[swarm:${cfg.key}] signal insert:`, error.message);
      // DEBUG: capture the exact body shape that PostgREST rejects, so we
      // can identify which field still contains invalid JSON content after
      // the stripCtrl pass. Temporary — remove once root cause is fixed.
      try {
        const safe = {};
        for (const [k, v] of Object.entries(body)) {
          if (typeof v === 'string') safe[k] = { _t: 'str', len: v.length, sample: JSON.stringify(v.slice(0, 80)), codes: [...v.slice(0, 40)].map(c => c.charCodeAt(0)) };
          else safe[k] = { _t: typeof v, val: JSON.stringify(v)?.slice(0, 200) };
        }
        console.error(`[swarm:${cfg.key}] DEBUG body:`, JSON.stringify(safe));
      } catch (e) { console.error(`[swarm:${cfg.key}] DEBUG body err:`, e.message); }
      return;
    }
