...
export function registerSwarm(app, deps) {
  ...
  async function maybePublishDailyAnalysis(cfg) {
    ...
    try {
      ...
      let parsed = null;
      try {
        const raw = await llmComplete([{ role: 'system', content: sys }, { role: 'user', content: usr }], { prefer: cfg.brain });
        parsed = parseLlmJson(raw);
      } catch (e) { console.warn(`[swarm:${cfg.key}] analysis LLM failed: ${e.message}`); }
