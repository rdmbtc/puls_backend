...
export function registerSwarm(app, deps) {
  ...
  async function nearTermMarkets() {
    ...
    const out = [];
    try {
      const r = await fetch('https://gamma-api.polymarket.com/markets?closed=false&active=true&order=volume&ascending=false&limit=150', { headers: { Accept: 'application/json' } });
      if (r.ok) {
  ...
  async function commentOnWorldCup(cfg) {
    try {
      ...
      let brief = '';
      try { const r = await researchQuestion(`${m.team} 2026 World Cup form chances`, 3); brief = r?.brief || ''; } catch (_) {}
      let text;
      try {
  ...
  async function reactToHumanTrades() {
    ...
    try {
      ...
      for (const ht of (trades || [])) {
        ...
        try {
          let brief = '';
          try { const r = await researchQuestion(question, 3); brief = r?.brief || ''; } catch (_) {}
  ...
  async function runTrader(cfg) {
    ...
    if (bestEdge < cfg.minEdge || stake < 0.1) {
      ...
      try {
        ...
        const sys = `You are ${cfg.name}, ${cfg.persona} You trade on Puls (Arc Testnet). Puls prices mirror the Polymarket consensus 1:1, so you do NOT look for price gaps between venues — you decide whether live web research + the consensus justify backing a side.${signalCtx ? ` You just reviewed a peer agent's signal (below) and decided to ${signalCtx.verdict} it.` : ''}${research.brief ? ' You researched the live web (below).' : ''}${cryptoPricesStr}${memory ? ' You also see YOUR OWN recent track record below — calibrate to it: repeat the kinds of calls that win, and cut the patterns that lose.' : ''} Pick the single best trade. STRICT JSON only: {"slug":"...","side":"YES"|"NO","reasoning":"<2 sentences in your voice, cite the consensus probability + your web finding — never compare on-chain vs Polymarket prices>"}`;
  ...
  async function backOwnCall(cfg, call) {
    ...
    try {
      ...
      await postComment(cfg, 'market', String(contractAddress), `Backing my own signal with real USDC — took ${call.stance} here. Skin in the game.`);
  ...
  async function runCreator(cfg) {
    ...
    if (cfg.category === 'worldcup') {
      ...
      const soonWc = cand
        ...
        .slice(0, 8);
      const wcPool = soonWc.length ? soonWc : cand;
      ...
      try {
        const res = await researchQuestion(`${m.question} 2026 FIFA World Cup`, 8);
      ...
      if (shortPick) {
        pick = shortPick;
        try {
          const res = await researchQuestion(pick.q, 8);
          researchBrief = res?.brief || '';
          researchSources = Array.isArray(res?.sources) ? res.sources.slice(0, 10) : [];
        } catch (_) {}
      } else {
    ...
    // ── Publish gate ──────────────────────────────────────────────────────────
    // Better no signal than a shallow one: require real sources + a long-enough
    // thesis. Skip this cycle and retry next tick if research/LLM underdelivered.
    const SOURCES_MIN = parseInt(process.env.AGENT_SIGNAL_SOURCES_MIN || '6', 10);
    const THESIS_MIN  = parseInt(process.env.AGENT_SIGNAL_THESIS_MIN  || '1500', 10);
    if (researchSources.length < SOURCES_MIN) {
      console.warn(`[swarm:${cfg.key}] skip publish: only ${researchSources.length} sources (need ${SOURCES_MIN})`);
      s.signalId = null; s.skipReason = 'insufficient_sources';
      return s;
    }
    if (!thesis || thesis.length < THESIS_MIN) {
      console.warn(`[swarm:${cfg.key}] skip publish: thesis too short (${thesis ? thesis.length : 0} < ${THESIS_MIN})`);
      s.signalId = null; s.skipReason = 'short_thesis';
      return s;
    }
  ...
  async function runOne(cfg) {
    ...
    try {
      if (cfg.role === 'creator') {
        ...
        if (cfg.alsoTrades && call) await backOwnCall(cfg, call);
      } else {
        await runTrader(cfg);
      }
    } catch (e) {
  ...
  function start() {
    ...
    creators.forEach((cfg, i) => {
      ...
      const everyMs = 90 * 60 * 1000 + i * 60 * 1000;        // ~90m, fill columns faster
      setTimeout(() => {
    ...
    ROSTER.forEach((cfg, i) => {
      ...
      const everyMs = 15 * 60 * 1000 + i * 17000;
      setTimeout(() => {
        maybeEngageBlog(cfg);
        setInterval(() => maybeEngageBlog(cfg), everyMs);
      }, firstMs);
      // Author side of the debate: reply to counter-takes / thank agreers on my
      // own posts. Offset after engagement so the opposing take has landed first.
      setTimeout(() => {
        maybeReplyToComments(cfg);
        setInterval(() => maybeReplyToComments(cfg), everyMs);
      }, firstMs + 11 * 60 * 1000);
    });
    // Agent→agent alpha market: every agent periodically buys a FRESH peer
    // signal (x402) — a dense, always-live machine economy. ~9m staggered.
    ROSTER.forEach((cfg, i) => {
      const firstMs = 4 * 60 * 1000 + i * 50 * 1000;
      const everyMs = 9 * 60 * 1000 + i * 13000;
      setTimeout(() => {
        maybeBuyPeerSignal(cfg);
        setInterval(() => maybeBuyPeerSignal(cfg), everyMs);
      }, firstMs);
    });
  ...
  app.post('/api/agents/chat', deps.authenticateUser, deps.requireVerifiedUser, deps.strictLimiter, async (req, res) => {
    try {
      ...
      let research = { brief: '' };
      try {
        research = await deps.researchQuestion(message.slice(0, 200), 2);
      } catch (e) {}
