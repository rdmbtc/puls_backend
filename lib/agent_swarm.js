...
⛓ https://testnet.arcscan.app/tx/${txId || 'pending...'}\n`);
  ...
  async function maybeExitPosition(cfg) {
    ...
    try {
      ...
      const seen = new Set();
      const contracts = [];
      // Resolved markets the agent traded come FIRST — claiming winnings is the
      // biggest revenue source and the old code only scanned the last 30 trades
      // (always fresh unresolved markets, so claims never fired).
      for (const t of (tradesRes.data || [])) {
        const c = t?.market_id;
        if (!c || !/^0x[0-9a-fA-F]{40}$/.test(c)) continue;
        const k = c.toLowerCase();
        if (!seen.has(k) && resolvedByAddr.has(k)) { seen.add(k); contracts.push(c); }
      }
      // Then open (unresolved) positions for sell management.
      for (const t of (tradesRes.data || [])) {
        const c = t?.market_id;
        if (!c || !/^0x[0-9a-fA-F]{40}$/.test(c)) continue;
        const k = c.toLowerCase();
