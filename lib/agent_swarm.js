...
⛓ https://testnet.arcscan.app/tx/${txId || 'pending...'}\n`);
  ...
  if (ROSTER.length === 0) {
    ...
    // Register stub routes so the frontend gets 200 + empty arrays, not 404.
    app.get('/api/agents/roster', (_req, res) => res.json({ enabled: false, count: 0, agents: [] }));
    app.get('/api/agents/feed', (_req, res) => res.json({ enabled: false, count: 0, events: [] }));
  ...
  async function worldCupMarkets() {
    ...
    try {
      ...
      const events = await fetchGamma('/events?limit=60&closed=false&active=true&order=volume&ascending=false&tag_slug=world-cup');
      ...
      if (!out.some(m => m.slug?.includes('win-the-2026-fifa-world-cup'))) {
        const e2 = await fetchGamma('/events?slug=world-cup-winner');
        if (e2?.[0]) pushFromEvent(e2[0]);
      }
    } catch (_) {}
  ...
  function start() {
    // ── One-time staggered boot kicks (replace the old setInterval cadence) ──
    ROSTER.forEach((cfg, i) => {
      setTimeout(() => runOne(cfg), 60 * 1000 + i * 90 * 1000);
    });
    creators.forEach((cfg, i) => {
      setTimeout(() => maybePublishDailyAnalysis(cfg), 3 * 60 * 1000 + i * 45 * 1000);
    });
    ROSTER.forEach((cfg, i) => {
    ...
    // ── React to human trades: a human trade completing IS the trigger ────
    eventBus.on(EVENTS.TRADE_COMPLETE, (t) => {
      if (t && t.user_id && !String(t.user_id).startsWith('agent_')) {
        // Small delay so the on-chain position settles before the swarm reacts.
        setTimeout(() => reactToHumanTrades().catch(() => {}), 120_000).unref?.();
      }
    });
