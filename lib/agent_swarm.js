...
import { computeKellySize } from './kelly.js';
import { eventBus, EVENTS } from './events.js';
import { cache } from './cache.js';
import { fetchGamma } from './polymarket_client.js';
import { retrieveContext, indexDecision } from './opensearch.js';
import { rememberAgent, rememberTeam, recallAgent, recallTeam } from './agent_memory.js';
...
export function buildSwarmRoster() {
  ...
  for (const a of roster) {
    ...
    if (m) a.brain = m;
    // Sanitize agent name (e.g. "Nova 🌐") to ensure emoji are well-formed
    // and won't produce lone surrogates if sliced in template strings.
    const cleanName = new TextDecoder('utf-8', { fatal: false }).decode(
      new TextEncoder().encode(a.name)
    );
    a.name = cleanName;
...
export function registerSwarm(app, deps) {
  ...
  // opts.agentKey: when set AND that agent runs on a Circle Agent Wallet
  // (no dev-controlled walletId), the same Memo call executes from the Agent
  // Wallet via the Circle CLI instead — signals, tips and research payments
  // all flow through Circle Agent Stack for those agents.
  async function usdcTransferWithMemo(walletId, toAddr, amountMicro, memoKey, memoObj, opts = {}) {
    ...
    // ── Circle Agent Stack path ───────────────────────────────────────────
    if (!walletId && opts.agentKey && circleAgent.isEnabled(opts.agentKey)) {
      const awAddress = circleAgent.addressFor(opts.agentKey);
      if (!awAddress) throw new Error(`CIRCLE_AGENT_WALLET_ADDRESS_${opts.agentKey.toUpperCase()} is not set`);
      const pending = await circleAgent.pendingCount(awAddress).catch(() => -1);
      if (pending > 0) throw new Error(`${pending} pending Circle tx(s) — deferring agent-wallet payment`);
      ...
      try {
        const res = await circleAgent.executeContract({
          signature: 'memo(address,bytes,bytes32,bytes)',
          params: [USDC, innerData, memoId, memoData],
          contract: MEMO_CONTRACT,
          address: awAddress,
        });
        return { txId: res.txHash || res.id || null, memo: true };
      } catch (e) {
        console.warn(`[swarm:${opts.agentKey}] memo transfer fell back to plain (Agent Wallet): ${e.message}`);
      }
      const plain = await circleAgent.executeContract({
        signature: 'transfer(address,uint256)',
        params: [toAddr, String(amountMicro)],
        contract: USDC,
        address: awAddress,
      });
      return { txId: plain.txHash || plain.id || null, memo: false };
    }
  ...
  async function x402Research(cfg, question, limit = 3) {
    // Circle Agent Stack agents pay for live market data through Circle
    // Nanopayments (x402) from their Agent Wallet — throttled internally.
    if (circleAgent.isEnabled(cfg.key)) {
      const awAddress = circleAgent.addressFor(cfg.key);
      if (awAddress) await circleAgent.buyMarketsSnapshot(awAddress);
    }
    try {
      if (cfg && cfg.walletKey && adminAccount && !circleAgent.isEnabled(cfg.key)) {
        ...
        if (walletId) {
          ...
          if (String(process.env.LOG_BANNERS || '').trim().toLowerCase() === 'false') {
            console.log(`[swarm] x402 cleared: 0.0001 USDC ${cfg.name} → Puls Data Faucet (tx ${txId || 'pending'})`);
          } else {
...
⛓ https://testnet.arcscan.app/tx/${txId || 'pending...'}\n`);
      ...
      try {
        ...
        const agents = await Promise.all(STATIC_ROSTER.map(async (cfg) => {
          const { data: rows } = await supabase
            .from('notifications').select('body, created_at')
  ...
  async function ensureAgent(cfg) {
    ...
    // ── Circle Agent Stack wallets (CIRCLE_AGENT_WALLETS=vega,...) ─────────
    // These agents skip the dev-controlled SCA flow entirely: their funds
    // live in a Circle Agent Wallet (user-controlled 2-of-2 MPC) and every
    // transaction goes through the Circle CLI — lib/circle_agent_wallet.js.
    // agentWalletId is null downstream; executeAgentTrade branches on it.
    if (circleAgent.isEnabled(cfg.key)) {
      const awAddress = circleAgent.addressFor(cfg.key);
      let balance = 0;
      try { balance = await circleAgent.usdcBalance(awAddress); }
      catch (e) { console.warn(`[swarm:${cfg.key}] agent-wallet balance failed: ${e.message}`); }

      await supabase.from('profiles').upsert({
        user_id: cfg.user,
        display_name: cfg.name,
        bio: `Autonomous ${cfg.role}-agent on Puls — ${cfg.persona} Runs on the Circle Agent Stack: Agent Wallet + Nanopayments on Arc Testnet.`,
        avatar_url: `https://api.dicebear.com/7.x/bottts/png?size=128&seed=${encodeURIComponent(cfg.key)}`,
      }, { onConflict: 'user_id' });

      // ERC-8004 identity from the Agent Wallet — OPT-IN via
      // CIRCLE_AGENT_8004_MINT=true. Off by default during the 2026-08-22
      // Circle execution-pipeline incident: every submit lands in INITIATED
      // and jams the wallet's queue (trades included).
      const want8004 = String(process.env.CIRCLE_AGENT_8004_MINT || '').toLowerCase() === 'true';
      if (want8004 && !s.registered && balance >= 0.2) {
        try {
          const pending = await circleAgent.pendingCount(awAddress).catch(() => -1);
          if (pending > 0) {
            console.warn(`[swarm:${cfg.key}] ${pending} pending Circle tx(s) — deferring ERC-8004 register`);
          } else {
          const existing = await resolveAgentTokenId(cfg.walletKey, awAddress);
          if (existing) {
            s.registered = true;
          } else {
            await circleAgent.executeContract({
              signature: 'register(string)', params: [AGENT_METADATA_URI],
              contract: IDENTITY_REGISTRY, address: awAddress,
            });
            await new Promise(r => setTimeout(r, 4000));
            const id = await resolveAgentTokenId(cfg.walletKey, awAddress);
            if (id) { s.registered = true; console.log(`[swarm:${cfg.key}] ERC-8004 identity ${id} minted from Circle Agent Wallet`); }
          }
          }
        } catch (e) { console.warn(`[swarm:${cfg.key}] agent-wallet ERC-8004:`, e.message); }
      }

      s.ensured = true;
      console.log(`[swarm:${cfg.key}] Circle Agent Wallet ${awAddress} (${balance.toFixed(2)} USDC)`);
      return { walletId: null, address: awAddress, balance };
    }

    // Race condition guard: if another call is already creating a wallet for
    // this agent, wait for it to finish (max 30s), then use the wallet it created.
    if (_walletCreating.has(cfg.key)) {
      let waited = 0;
      while (_walletCreating.has(cfg.key) && waited < 300) {
        await new Promise(r => setTimeout(r, 100));
        waited++;
      }
      if (_walletCreating.has(cfg.key)) {
        console.warn(`[swarm:${cfg.key}] wallet creation wait timed out — proceeding to create`);
      } else {
        const walletId = await getWalletId(cfg.walletKey);
        if (walletId) {
    ...
    if (!walletId) {
      // Double-checked locking: add our key first, then re-check in case
      // another concurrent call already created the wallet between our check
      // and the add.
      if (_walletCreating.has(cfg.key)) {
        // Another call beat us to it — wait for theirs to finish
        let waited = 0;
        while (_walletCreating.has(cfg.key) && waited < 300) {
          await new Promise(r => setTimeout(r, 100));
          waited++;
        }
        walletId = await getWalletId(cfg.walletKey);
      } else {
    ...
    // Investor capital counts toward the target: active sponsorships raise the
    // bankroll the agent trades with (target = bootstrap + invested), so
    // invested USDC is actually put to work after restarts too.
    let investedSum = 0;
    try {
      const { data: invs } = await supabase
        .from('investments')
        .select('amount_usdc')
        .eq('agent_id', cfg.user)
        .eq('status', 'active');
      investedSum = (invs || []).reduce((s, i) => s + Number(i.amount_usdc || 0), 0);
    } catch (e) {
      console.warn(`[swarm:${cfg.key}] investments read failed:`, e.message);
    }
    const baseTarget = (cfg.role === 'creator' && !cfg.alsoTrades) ? Math.min(BOOTSTRAP_USDC, 0.6) : BOOTSTRAP_USDC;
    const target = baseTarget + investedSum;
  ...
  async function pickPeerSignal(cfg) {
    ...
    const { data: rows } = await supabase
      ...
      .select('id, creator_user_id, title, market_question, market_slug, stance, confidence, edge_bps, horizon, thesis, price_usdc, onchain_tx')
      .eq('status', 'published')
    ...
    // Exclude signals on markets that have resolved (stale alpha guard)
    ...
    if (slugs.length) {
      try {
        const { data: dm } = await supabase
          ...
          .select('slug, resolved')
          .in('slug', slugs);
        (dm || []).forEach(m => { if (m.resolved === true) resolvedSet.add(m.slug); });
      } catch (_) {}
    }
    const unresolvedRows = rows.filter(r => !r.market_slug || !resolvedSet.has(r.market_slug));
    if (!unresolvedRows.length) return null;
    ...
    try {
      const { data: mine } = await supabase.from('signal_unlocks')
        .select('signal_id').eq('user_id', cfg.user).in('signal_id', unresolvedRows.map((r) => r.id));
      owned = new Set((mine || []).map((r) => r.signal_id));
    } catch (_) {}
    const fresh = unresolvedRows.filter((r) => !owned.has(r.id));
    if (!fresh.length) return null;
    return fresh[Math.floor(Math.random() * Math.min(fresh.length, 5))];
  }

  async function creatorWalletAddress(creatorUserId) {
    // Circle Agent Stack creators receive on their Agent Wallet — this MUST
    // match the payTo shown in /api/x402/signals discovery, or internal
    // buyers and external buyers would pay different addresses.
    if (circleAgent.isEnabledForUser(creatorUserId)) {
      return circleAgent.addressFor(circleAgent.keyFromUser(creatorUserId)) || null;
    }
  ...
  // Agent-Stack agents pay from their Agent Wallet (memo path routes via the
  // Circle CLI automatically — see usdcTransferWithMemo opts.agentKey).
  async function buySignal(cfg, buyerWalletId, buyerAddress, signal) {
    ...
    try {
      ...
      }, { onConflict: 'user_id, signal_id', ignoreDuplicates: true }).then(({ error }) => { if (error && !String(error.message).includes('duplicate')) console.warn(`[swarm:${cfg.key}] unlock insert:`, error.message); }).catch(e => console.warn(`[swarm:${cfg.key}] unlock insert network error:`, e.message));
      supabase.from('creator_signals').select('unlocks_count, revenue_usdc').eq('id', signal.id).maybeSingle()
        ...
        }).eq('id', signal.id).then(() => {}).catch(e => console.warn(`[swarm:${cfg.key}] signal update error:`, e.message)); })
        .catch(e => console.warn(`[swarm:${cfg.key}] signal select error:`, e.message));
      ...
      }).then(({ error }) => { if (error) console.warn(`[swarm:${cfg.key}] x402 receipt:`, error.message); }).catch(e => console.warn(`[swarm:${cfg.key}] x402 insert network error:`, e.message));
  ...
  const _agentCommentDedupe = new Map(); // dedupeKey -> timestamp
  async function postComment(cfg, targetType, targetId, body, parentId = null) {
    try {
      const dedupeKey = `${cfg.user}:${targetType}:${targetId}`;
      const now = Date.now();
      const last = _agentCommentDedupe.get(dedupeKey);
      // Prevent duplicate comments on the same target within 24 hours
      if (last && (now - last < 24 * 60 * 60 * 1000)) {
        return;
      }
      _agentCommentDedupe.set(dedupeKey, now);
      if (_agentCommentDedupe.size > 2000) {
        for (const [k, ts] of _agentCommentDedupe.entries()) {
          if (now - ts > 24 * 60 * 60 * 1000) _agentCommentDedupe.delete(k);
        }
      }
  ...
  const reactedHumanTrades = new Map(); // tradeId -> timestamp (pruned to 60min)
  ...
  async function reactToHumanTrades() {
    ...
    try {
      ...
      // Prune stale entries (older than 60 min) to prevent unbounded growth
      const pruneCutoff = Date.now() - 60 * 60 * 1000;
      for (const [id, ts] of reactedHumanTrades) {
        if (ts < pruneCutoff) reactedHumanTrades.delete(id);
      }
      let done = 0;
      for (const ht of trades) {
        if (done >= REACT_MAX_PER_CYCLE) break;
        if (!ht.market_id || reactedHumanTrades.has(ht.id)) continue;
        reactedHumanTrades.set(ht.id, Date.now());
  ...
  const SOLD_EVENT_ABI = [{ name: 'Sold', type: 'event', inputs: [
    { indexed: true, name: 'user', type: 'address' }, { indexed: false, name: 'side', type: 'bool' },
    { indexed: false, name: 'shares', type: 'uint256' }, { indexed: false, name: 'usdcOut', type: 'uint256' } ] }];
  const CLAIMS_PER_CYCLE = 25;

  // Market info only changes when a market resolves — cache it hard (5 min)
  // so the exit cycle doesn't re-read the same 40 contracts every sweep.
  const _marketInfoCache = new Map(); // addr(lower) -> { info, at }
  async function getMarketInfoCached(contract) {
    const k = String(contract).toLowerCase();
    const hit = _marketInfoCache.get(k);
    if (hit && Date.now() - hit.at < 5 * 60 * 1000) return hit.info;
    const info = await publicClient.readContract({ address: contract, abi: INFO_ABI, functionName: 'getMarketInfo' });
    _marketInfoCache.set(k, { info, at: Date.now() });
    return info;
  }
  // Global serialization: only one agent's exit cycle runs at a time. Six
  // agents each scanning 40 contracts concurrently was bursting the Arc RPC.
  let _exitChain = Promise.resolve();
  function serializeExit(fn) {
    const p = _exitChain.then(fn, fn);
    _exitChain = p.catch(() => {});
    return p;
  }

  async function maybeExitPosition(cfg) {
    return serializeExit(async () => {
    if (!AGENT_SELL || !publicClient) return;
    try {
      const agent = await ensureAgent(cfg);
      if (!agent.address) return;
      const [resolvedRes, tradesRes] = await Promise.all([
        supabase.from('deployed_markets').select('contract_address, outcome').eq('resolved', true),
        supabase.from('trades').select('market_id, side')
          .eq('user_id', cfg.user).eq('state', 'COMPLETE')
          .order('created_at', { ascending: false }).limit(60),
      ]);
      const resolvedByAddr = new Map();
      for (const m of (resolvedRes.data || [])) {
        if (m.outcome === null || m.outcome === undefined) continue;
        resolvedByAddr.set(String(m.contract_address || '').toLowerCase(), Boolean(m.outcome));
      }
      // Resolved markets the agent traded — claims are the biggest revenue
      // source and must come FIRST. The agent's recent trades are always fresh
      // unresolved markets, so the old "last 30 trades" scan never saw them.
      let resolvedContracts = [];
      try {
        if (supabase.pool) {
          const { rows } = await supabase.pool.query(
            `SELECT DISTINCT t.market_id AS contract_address FROM trades t
             JOIN deployed_markets dm ON dm.contract_address = t.market_id
             WHERE t.user_id = $1 AND t.state = 'COMPLETE'
               AND dm.resolved = true AND dm.outcome IS NOT NULL
               AND NOT EXISTS (SELECT 1 FROM trades c WHERE c.user_id = t.user_id
                 AND c.market_id = t.market_id AND c.side = 'CLAIM')
             LIMIT 40`,
            [cfg.user]
          );
          resolvedContracts = rows.map((r) => r.contract_address).filter((c) => /^0x[0-9a-fA-F]{40}$/.test(c || ''));
        }
      } catch (e) {
        console.warn(`[swarm:${cfg.key}] resolved-markets query failed: ${e.message}`);
      }
      const seen = new Set();
      const contracts = [];
      ...
      // Then open (unresolved) positions from recent trades for sell management.
      for (const t of (tradesRes.data || [])) {
        const c = t?.market_id;
        if (!c || !/^0x[0-9a-fA-F]{40}$/.test(c)) continue;
        const k = c.toLowerCase();
        if (seen.has(k) || resolvedByAddr.has(k)) continue;
        seen.add(k);
        contracts.push(c);
      }
      if (!contracts.length) return;

      const slice = contracts.slice(0, 40);
      // Batch every position read into ONE multicall per agent and serve
      // market info from cache — this cut ~480 RPC calls per full cycle down
      // to ~2. The Arc RPC was rate-limiting the swarm flat ("Request exceeds
      // defined limit"), which also broke the market resolver.
      const posBatch = await Promise.all(slice.map((c) =>
        publicClient.readContract({ address: c, abi: POS_ABI, functionName: 'getUserPosition', args: [agent.address] })
          .catch(() => null)));
      const infoBatch = await Promise.all(slice.map((c) => getMarketInfoCached(c).catch(() => null)));
      let claimedThisCycle = 0;
      for (let i = 0; i < slice.length; i++) {
        const contract = slice[i];
        const pos = posBatch[i], info = infoBatch[i];
        ...
        if (resolved) {
          ...
          const payout = Math.max(yes, no);
          if (claimedThisCycle >= CLAIMS_PER_CYCLE) continue;
          const { data: claimRow } = await supabase
            .from('trades').select('id').eq('user_id', cfg.user)
            .eq('market_id', contract).eq('side', 'CLAIM').limit(1);
          const alreadyRecorded = claimRow && claimRow.length > 0;
          try {
            let claimResult;
            if (circleAgent.isEnabledForUser(cfg.user)) {
              // Circle Agent Stack: claim() from the Agent Wallet via CLI
              const res = await circleAgent.executeContract({
                signature: 'claim()', params: [], contract,
                address: circleAgent.addressFor(circleAgent.keyFromUser(cfg.user)),
              });
              claimResult = { data: { id: res.id } };
            } else {
              const txRes = await circle.createContractExecutionTransaction({
                walletId: agent.walletId, contractAddress: contract,
                abiFunctionSignature: 'claim()', abiParameters: [],
                fee: { type: 'level', config: { feeLevel: 'HIGH' } },
              });
              claimResult = txRes;
            }
            if (!alreadyRecorded) {
              await supabase.from('trades').insert({
                user_id: cfg.user,
                tx_id: claimResult.data?.id || `claim_${Date.now()}`,
                side: 'CLAIM',
                usdc_amount: +payout.toFixed(6),
                entry_price: 0,
                question: `Claim Winnings: ${String(info[0] || contract).slice(0, 90)}`,
                market_id: contract,
                state: 'COMPLETE',
              });
            }
            claimedThisCycle++;
            // Only surface the first claim of the cycle in the feed — bulk
            // catch-up claims would otherwise flood notifications.
            if (claimedThisCycle === 1) {
              await supabase.from('notifications').insert({
                user_id: cfg.user, title: String(info[0] || contract), type: 'agent_decision', read: true,
                message: JSON.stringify({
                  action: 'claim', agentKey: cfg.key, agentName: cfg.name, role: 'trader',
                  side, contractAddress: contract,
                  reasoning: `Claimed my winning ${side} position (+${payout.toFixed(4)} USDC) on a resolved market.`,
                }),
              });
            }
            console.log(`[swarm:${cfg.key}] claimed ${side} on resolved ${contract} (+${payout.toFixed(4)} USDC)`);
          } catch (e) { console.warn(`[swarm:${cfg.key}] claim failed on ${contract}:`, e.message); }
          continue;
        }
        if (claimed) continue;
        const side = yes > no ? 'YES' : 'NO';
        ...
        try {
          ...
          let txHash = null, finalState = null, usdcOut = null;
          for (let i = 0; i < 25; i++) {
          ...
          if (finalState === 'COMPLETE' && txHash) {
            try {
              const receipt = await publicClient.getTransactionReceipt({ hash: txHash });
              for (const log of (receipt?.logs || [])) {
                if (String(log.address).toLowerCase() !== String(contract).toLowerCase()) continue;
                try {
                  const decoded = decodeEventLog({ abi: SOLD_EVENT_ABI, data: log.data, topics: log.topics });
                  if (decoded.eventName === 'Sold' && decoded.args) usdcOut = Number(decoded.args.usdcOut) / 1e6;
                } catch (_) {}
              }
            } catch (e) { console.warn(`[swarm:${cfg.key}] sell receipt decode failed: ${e.message}`); }
            if (usdcOut != null && usdcOut > 0) {
              ...
              if (!dup || dup.length === 0) {
                await supabase.from('trades').insert({
                  ...
                  side,
                  usdc_amount: -usdcOut,
                  entry_price: shares > 0 ? +(usdcOut / shares).toFixed(4) : 0.5,
                  question: `Sold: ${String(info[0] || contract).slice(0, 90)}`,
                  market_id: contract,
                  state: 'COMPLETE',
                  tx_hash: txHash,
                });
                console.log(`[swarm:${cfg.key}] recorded sell ${side} ${usdcOut.toFixed(4)} USDC on ${contract}`);
              }
            } else {
              console.warn(`[swarm:${cfg.key}] sell ${side} on ${contract} confirmed but no Sold payout decoded`);
            }
          }
          await supabase.from('notifications').insert({
            user_id: cfg.user, title: String(info[0] || contract), type: 'agent_decision', read: true,
  ...
  async function runTrader(cfg) {
    ...
    let ragHits = 0; // RAG sections retrieved for this decision (outer scope — payload needs it)
    ...
    if (bestEdge < cfg.minEdge || stake < 0.1) {
      ...
      try {
        ...
        try {
          ...
          ragHits = (ragText.match(/\[RAG Semantic Context/g) || []).length;
        } catch (_) {}

        // Long-term memory (Mem0): the agent's distilled facts + shared swarm
        // lessons for this market. Terse, prompt-ready, no-op when disabled.
        let ltmText = '';
        try {
          const mine = await recallAgent(cfg.key, top[0].question, 4);
          const team = await recallTeam(top[0].question, 3);
          if (mine) ltmText += `\n\n[Long-Term Memory — your past lessons]:\n${mine}`;
          if (team) ltmText += `\n\n[Long-Term Memory — swarm lessons]:\n${team}`;
          if (ltmText) console.log(`[swarm:${cfg.key}] long-term memory → ${ltmText.length} chars`);
        } catch (_) {}
...
{"slug":"...","side":"YES"|"NO","confidence":<0-100 integer>,"reasoning":"<2-3 sentences in your voice. MUST include: (1) cite the consensus probability, (2) cite at least one specific web finding with source name, (3) explain your edge. Be concrete, not generic.>","counterargument":"<1 sentence: what's the strongest reason you could be WRONG?>","riskNote":"<1 sentence: if you're wrong, you lose the stake — how much USDC and what's the downside?>"}`;
        ...
        const raw = await llmComplete([{ role: 'system', content: sys }, { role: 'user', content: cText + sText + rText + mText + ltmText }], { prefer: cfg.brain });
        ...
        // Dynamic Fractional Kelly position sizing based on agent's estimated probability
        const kelly = computeKellySize({
          confidence: confidence / 100,
          marketPrice: side === 'YES' ? (chosen.pmYes || 0.5) : (1 - (chosen.pmYes || 0.5)),
          side,
          bankrollUsdc: agent.balance || 10.0,
          maxFraction: 0.08,
          minBet: 0.05,
          maxBet: 5.0,
        });
        const dynamicAmount = kelly.shouldTrade ? kelly.amountUsdc : stake;

        decision = { ...chosen, action: 'go', side, amount: dynamicAmount, brain: 'AI', confidence,
    ...
    // ragHits is filled during retrieval above (outer scope) — surfaced so
    // the office floor can show the agent visiting the SERVER ROOM.
    const payload = {
    ...
    try {
      const { data: prev } = await supabase
        .from('notifications').select('body')
    ...
    // Long-term memory: distill this decision into per-agent + swarm facts
    ...
    const memoryNote = {
      ...
      confidence: decision.confidence,
      reasoning: decision.reasoning,
      pmYes: decision.pmYes != null ? decision.pmYes : null,
      slug: decision.slug || null,
      outcome: null,
    };
    rememberAgent(cfg.key, memoryNote).catch(() => {});
    rememberTeam(memoryNote).catch(() => {});
  ...
  async function runCreator(cfg) {
    ...
    try {
      ...
      if (raw && raw.length > 20) {
        // Sanitize BEFORE slicing — if the LLM output contains an emoji that
        // straddles the 3500-char boundary, slicing first would break the
        // surrogate pair and create a lone high surrogate that formatForApp's
        // UTF-8 round-trip can't fix (it's already broken).
        const sanitizedRaw = new TextDecoder('utf-8', { fatal: false }).decode(
          new TextEncoder().encode(raw)
        );
        thesis = formatForApp(sanitizedRaw.slice(0, 3500));
      }
    } catch (_) {}
    ...
    // Sanitize every string BEFORE the insert. PostgREST rejects the whole
    // row with "invalid input syntax for type json" when any string contains
    // ill-formed Unicode (lone surrogates, non-characters, etc.). LLM output
    // and scraped web content are the main sources. A UTF-8 round-trip via
    // TextEncoder/TextDecoder is the most thorough fix — it replaces all
    // malformed sequences with U+FFFD, guaranteeing well-formed Unicode.
    const sanitizeUnicode = (v) => {
    ...
    const cleanSources = (researchSources || []).map((s) => ({
      title: sanitizeUnicode(s?.title), url: sanitizeUnicode(s?.url), source: sanitizeUnicode(s?.source),
    }));
    const body = {
      creator_user_id: cfg.user, title: sanitizeUnicode(pick.t), market_question: sanitizeUnicode(pick.q), stance: pick.s,
      market_slug: pick.marketSlug || null,
      confidence: pick.c, edge_bps: pick.e, horizon: pick.h,
      teaser: sanitizeUnicode(`${cfg.name} has a ${cleanSources.length}-source researched call on "${pick.t}". Unlock to see the side + the full thesis.`),
      thesis: sanitizeUnicode(thesis), price_usdc: 0.001, status: 'published', published_at: new Date().toISOString(),
    ...
    try {
      ...
      for (const heavy of [true, false]) {
        try {
          ...
          console.warn(`[swarm:${cfg.key}] analysis LLM (${heavy ? 'heavy' : 'fast'}) failed: ${e.message}`);
          if (heavy) {
            // Immediate retry with focused/truncated context to save the daily post
            try {
              const focusedUsr = `Today's beat: ${topic.q}\n\nKey grounded facts:\n${brief.slice(0, 1500)}`;
              const retryRaw = await llmComplete([{ role: 'system', content: sys }, { role: 'user', content: focusedUsr }], { prefer: cfg.brain, heavy: false });
              if (retryRaw && retryRaw.length > 50) {
                try { parsed = parseLlmJson(retryRaw); } catch (_) {}
                if (!parsed || !parsed.title || !parsed.body) {
                  const titleMatch = retryRaw.match(/#+\s*(.+)/) || retryRaw.match(/title["']?\s*:\s*["']([^"']+)["']/i);
                  const title = titleMatch ? titleMatch[1].replace(/["'{}]/g, '').trim() : `${topic.label || 'Market'} Analysis Report`;
                  const cleanBody = retryRaw.replace(/```json|```/g, '').trim();
                  parsed = { title, body: cleanBody, excerpt: cleanBody.slice(0, 250) + '...' };
                }
                if (parsed && parsed.title && parsed.body) break;
              }
  ...
  // ── Global concurrency semaphore: max 2 agents at once ──────────────
  // Prevents memory blowout on Heroku 512MB dyno when all 6 agents fire
  // simultaneous LLM calls + x402 payments + web research.
  let _activeAgents = 0;
  const MAX_CONCURRENT = 2;
  const _semaQueue = [];
  function acquireSema() {
    return new Promise((resolve) => {
      if (_activeAgents < MAX_CONCURRENT) {
        _activeAgents++;
        resolve();
      } else {
        _semaQueue.push(resolve);
      }
    });
  }
  function releaseSema() {
    if (_semaQueue.length) {
      const next = _semaQueue.shift();
      next();
    } else if (_activeAgents > 0) {
      _activeAgents--;
    }
  }
  ...
  async function runOne(cfg) {
    const s = st(cfg.key);
    // If busy flag is stale (set > 10 min ago), clear it so the agent can recover
    if (s.busy && s.busySince && Date.now() - s.busySince > 10 * 60 * 1000) {
      console.warn(`[swarm:${cfg.key}] busy flag stale (${Math.round((Date.now() - s.busySince) / 1000)}s) — clearing`);
      s.busy = false;
      s.busySince = 0;
    }
    if (s.busy) return;
    ...
    } finally { s.busy = false; s.busySince = 0; releaseSema(); }
  }
  ...
  const _startCleanup = []; // cleanup fns for eventBus listeners + intervals
  function start() {
    _startCleanup.length = 0; // reset if start is called again
    ...
    function onEvent(event, handler) {
      ...
      _startCleanup.push(() => eventBus.off(event, handler));
    }
    ...
    onEvent(EVENTS.TRADE_COMPLETE, onMarketActivity);
    onEvent(EVENTS.MARKET_ACTIVATED, onMarketActivity);
    ...
    onEvent(EVENTS.TRADE_COMPLETE, (t) => {
    ...
    onEvent(EVENTS.SIGNAL_PUBLISHED, () => {
    ...
    // ── Warm up: staggered boot at lines above already handles initial runs ──
    // No separate surge — prevents memory blowout on Heroku 512MB dyno.

    const _creatorTickInterval = setInterval(() => {
      creators.forEach((cfg) => setTimeout(() => runOne(cfg).catch(() => {}), 0));
    }, 15 * 60 * 1000);
    _startCleanup.push(() => clearInterval(_creatorTickInterval));
  ...
  app.get('/api/agents/roster', async (req, res) => {
    try {
      ...
      const allUsers = ROSTER.map((c) => c.user).filter(Boolean);

      // Batch fetch recent decisions for all agents in 1 query instead of N queries
      const { data: allNotifs } = await supabase
        ...
        .order('created_at', { ascending: false })
        .limit(ROSTER.length * 6);

      const notifsByUser = new Map();
      for (const r of allNotifs || []) {
        if (!notifsByUser.has(r.user_id)) notifsByUser.set(r.user_id, []);
        const list = notifsByUser.get(r.user_id);
        if (list.length < 6) list.push(r);
      }

      // Batch fetch latest creator signals
      const creatorUsers = ROSTER.filter((c) => c.role === 'creator').map((c) => c.user);
      const { data: allSignals } = await supabase
        .from('creator_signals')
        .select('id, creator_user_id, title, unlocks_count, revenue_usdc, onchain_tx, created_at')
        .in('creator_user_id', creatorUsers)
        .eq('status', 'published')
        .order('created_at', { ascending: false });

      const signalByCreator = new Map();
      for (const s of allSignals || []) {
        if (!signalByCreator.has(s.creator_user_id)) {
          signalByCreator.set(s.creator_user_id, s);
        }
      }

      const agents = await Promise.all(ROSTER.map(async (cfg) => {
        // ── Circle Agent Stack agents: report the AGENT WALLET, not SCA ────
        if (circleAgent.isEnabled(cfg.key)) {
          const awAddress = circleAgent.addressFor(cfg.key);
          let awBalance = 0;
          try { awBalance = await circleAgent.usdcBalance(awAddress); }
          catch (e) { console.warn(`[swarm:${cfg.key}] roster balance failed:`, e.message); }
          let erc8004Id = agentTokenIds.get(cfg.walletKey) ?? null;
          if (erc8004Id == null && typeof resolveAgentTokenId === 'function') {
            try { erc8004Id = await resolveAgentTokenId(cfg.walletKey, awAddress); } catch (_) {}
          }
          const rowsA = notifsByUser.get(cfg.user) || [];
          const decisionsA = (rowsA || []).map(r => { try { const msg = r.body || r.message; const d = typeof msg === 'object' ? msg : JSON.parse(msg); if (d.brain) d.brain = 'AI'; return { ...d, at: r.created_at }; } catch { return null; } }).filter(Boolean);
          let signalA = null;
          if (cfg.role === 'creator') {
            const sig = signalByCreator.get(cfg.user);
            if (sig) signalA = { id: sig.id, title: sig.title, unlocks: sig.unlocks_count ?? 0, revenueUsdc: Number(sig.revenue_usdc ?? 0), onchainTx: sig.onchain_tx };
          }
          return {
            key: cfg.key, name: cfg.name, role: cfg.role, brain: 'AI', persona: cfg.persona,
            strategy: cfg.strategy || 'balanced',
            address: awAddress, balance: awBalance, erc8004Id,
            circleWallet: true,
            recentDecisions: decisionsA, signal: signalA,
          };
        }
        ...
        const rows = notifsByUser.get(cfg.user) || [];
        const decisions = (rows || []).map(r => { try { const msg = r.body || r.message; const d = typeof msg === 'object' ? msg : JSON.parse(msg); if (d.brain) d.brain = 'AI'; return { ...d, at: r.created_at }; } catch { return null; } }).filter(Boolean);
        let signal = null;
        if (cfg.role === 'creator') {
          const sig = signalByCreator.get(cfg.user);
    ...
    try {
      ...
      const events = (rows || []).map(r => {
        ...
        return {
          ...
          // RAG engine usage — drives the office's SERVER ROOM visits
          ragHits: m.ragHits ?? 0,
...
${research.brief ? `\nLive web research context to ground your answer:\n${research.brief}\n` : ''}`;
  ...
  function stop() {
    for (const fn of _startCleanup) try { fn(); } catch (e) { console.warn('[swarm] cleanup error:', e.message); }
    _startCleanup.length = 0;
  }
  return { tick: async () => { for (const c of ROSTER) await runOne(c); }, start, stop, roster: ROSTER };
}
