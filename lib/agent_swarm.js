          ...
          try {
            ...
            await supabase.from('notifications').insert({
              ...
              message: JSON.stringify({
                ...
                reasoning: `Claimed my winning ${side} position on a resolved market.`,
              }),
            });
            await postComment(cfg, 'market', String(info[0] || contract), `Claimed my ${side} winnings.`);
            console.log(`[swarm:${cfg.key}] claimed ${side} on resolved ${contract}`);
            return;
