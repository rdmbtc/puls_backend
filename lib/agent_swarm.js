  ...
  async function runTrader(cfg) {
    ...
    if (bestEdge < cfg.minEdge || stake < 0.1) {
      ...
      try {
        ...
        // [Puls Gateway] Динамический выбор нужного API в зависимости от темы рынка
        const questionLower = (top[0].question || '').toLowerCase();
        const requiresMacro = questionLower.includes('fed') || questionLower.includes('rate') || questionLower.includes('inflation');
        const targetEndpoint = requiresMacro ? 'sugra_macro' : 'btcnode_premium';

        // [Puls Gateway] Attempt to buy premium data before making a decision
        const gatewayResult = await evaluateAndBuyData(
          ...
          targetEndpoint // Передаем динамически выбранный эндпоинт
        );
        let gatewayStr = '';
        ...
        const sys = `You are ${cfg.name}, ${cfg.persona} You trade on Puls (Arc Testnet). Puls prices mirror the Polymarket consensus 1:1, so you do NOT look for price gaps between venues — you decide whether live web research + the consensus justify backing a side.${signalCtx ? ` You just reviewed a peer agent's signal (below) and decided to ${signalCtx.verdict} it.` : ''}${research.brief ? ' You researched the live web (below).' : ''}${cryptoPricesStr}${gatewayStr}${memory ? ' You also see YOUR OWN recent track record below — calibrate to it: repeat the kinds of calls that win, and cut the patterns that lose.' : ''} Pick the single best trade. STRICT JSON only: {"slug":"...","side":"YES"|"NO","reasoning":"<2 sentences in your voice, cite the consensus probability + your web finding — never compare on-chain vs Polymarket prices>"}`;
