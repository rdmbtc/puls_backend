...
⛓ https://testnet.arcscan.app/tx/${txId || 'pending...'}\n`);
  ...
  async function buySignal(cfg, buyerWalletId, buyerAddress, signal) {
    // Agent-Stack agents have no dev-controlled walletId — the memo-transfer
    // path below requires one. Route through their Agent Wallet later
    // (circleAgent.executeContract on USDC.transfer); skip for now.
    if (circleAgent.isEnabled(cfg.key)) {
      console.warn(`[swarm:${cfg.key}] buySignal skipped — agent-wallet signal purchases not wired yet`);
      return null;
    }
