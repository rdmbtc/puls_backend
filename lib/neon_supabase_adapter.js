...
class NeonQueryBuilder {
  ...
  or(condStr) {
    if (!condStr) return this;
    const parts = condStr.split(',').map(s => s.trim()).filter(Boolean);
    const subConds = [];
    for (const part of parts) {
      const m = part.match(/^([^.]+)\.(eq|ilike|like|neq)\.(.+)$/);
      if (m) {
        let [, col, op, val] = m;
        val = val.replace(/^"|"$/g, '');
        this.whereValues.push(val);
        const pIdx = this.whereValues.length;
        const sqlOp = op === 'eq' ? '=' : (op === 'neq' ? '!=' : op.toUpperCase());
        subConds.push(`"${col}" ${sqlOp} $${pIdx}`);
      }
    }
    if (subConds.length) {
...
const formatParam = (v) => {
          if (Array.isArray(v)) {
            ...
            return `{${v.map(e => `"${String(e).replace(/"/g, '""')}"`).join(',')}}`;
          }
      ...
      if (this.operation === 'UPSERT') {
        ...
        const formatParam = (v) => {
          if (Array.isArray(v)) {
            if (v.length && typeof v[0] === 'object' && v[0] !== null) return JSON.stringify(v);
            return `{${v.map(e => String(e).includes(',') || String(e).includes('"') ? `"${String(e).replace(/"/g, '""')}"` : String(e)).join(',')}}`;
          }
          return typeof v === 'object' && v !== null ? JSON.stringify(v) : v;
        };
...
export function createNeonClient(connectionString, realSupabaseClient = null) {
  // TLS note: connections are ENCRYPTED but not certificate-verified. Aiven's
  // legacy "Project CA" is self-signed WITHOUT the basicConstraints CA:TRUE
  // extension, so OpenSSL cannot treat it as a trust anchor (any pinned-CA
  // attempt fails with SELF_SIGNED_CERT_IN_CHAIN — verified empirically).
  // This only weakens THIS db connection; every other outbound call (Circle,
  // LLMs, OAuth, research APIs) runs with full certificate verification since
  // the global NODE_TLS_REJECT_UNAUTHORIZED=0 was removed.
  // To go strict: enable public/Let's-Encrypt service certificates for the
  // Aiven Postgres service (console → service → Advanced config), then set
  // DATABASE_CA_CERT to the new CA pem here.
  let ssl = { rejectUnauthorized: false };
  const caPath = process.env.DATABASE_CA_CERT;
  if (caPath) {
    try {
      ssl = { ca: fsSync.readFileSync(caPath), rejectUnauthorized: true };
    } catch (_) { /* fall back to unverified TLS */ }
  }
  // `sslmode=require` must be stripped from the URL — newer pg-connection-string
  // treats require/prefer/verify-ca as verify-full and its parsed settings
  // OVERRIDE an explicitly provided ssl object, which broke connections once
  // the global NODE_TLS_REJECT_UNAUTHORIZED=0 was removed from the environment.
  ...
  } catch (_) { /* not a parseable URL — leave as-is */ }
  const pool = new Pool({
  ...
  // Guard against unhandled error events on idle clients (e.g. serverless DB suspend or network disconnect)
  pool.on('error', (err) => {
    console.error('[neon/pg pool] unexpected error on idle client:', err?.message || err);
  });
