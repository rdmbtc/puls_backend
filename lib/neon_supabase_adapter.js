...
export function createNeonClient(connectionString, realSupabaseClient = null) {
  // TLS: prefer STRICT certificate verification. The bundled Aiven Project CA
  // (certs/aiven-project-ca.pem, captured from the service chain — CA certs are
  // public material) pins our managed Postgres; DATABASE_CA_CERT overrides it.
  // Only when no CA is available do we fall back to encryption-without-
  // verification (previous behavior) so other deployments keep working.
  let ssl = { rejectUnauthorized: false };
  try {
    const caPath = process.env.DATABASE_CA_CERT
      || path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'certs', 'aiven-project-ca.pem');
