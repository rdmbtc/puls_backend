 ...
 * evaluated first); local entries are never dropped.
 */
async function searchBazaarAsync(query) {
  const remote = await fetchDiscoveryServices();
  return bestOf([...remote, ...Object.values(PULS_REGISTRY)], query);
}
...
export { PULS_REGISTRY, evaluateAndBuyData, formatGatewayDataForPrompt, searchBazaar, searchBazaarAsync, mapDiscoveryResource, mapDiscoveryItems };
